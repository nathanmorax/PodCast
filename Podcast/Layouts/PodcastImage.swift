//
//  PodcastImage.swift
//  Podcast
//
//  Created by Satori Tech 341 on 11/05/26.
//
import SwiftUI
import UIKit

/// Muestra una imagen desde una URL remota, un asset local, o un placeholder.
/// Detecta automáticamente el tipo de fuente según el string que recibe.
struct PodcastImage: View {
    
    let source: String?
    var contentMode: ContentMode = .fill
    var placeholder: Color = Color(uiColor: .secondarySystemFill)
    
    var body: some View {
        if let source, !source.isEmpty {
            if source.hasPrefix("http"), let url = URL(string: source) {
                RemotePodcastImage(
                    url: url,
                    contentMode: contentMode,
                    placeholder: placeholder
                )
            } else {
                Image(source)
                    .resizable()
                    .aspectRatio(contentMode: contentMode)
            }
        } else {
            placeholder
        }
    }
}

// MARK: - Imagen remota con cache

private struct RemotePodcastImage: View {
    
    let url: URL
    let contentMode: ContentMode
    let placeholder: Color
    
    @State private var image: UIImage?
    @State private var loadTask: Task<Void, Never>?
    
    var body: some View {
        Group {
            if let image {
                Image(uiImage: image)
                    .resizable()
                    .aspectRatio(contentMode: contentMode)
                    .transition(.opacity)
            } else {
                placeholder
            }
        }
        .onAppear {
            loadTask = Task {
                await loadImage()
            }
        }
        .onDisappear {
            loadTask?.cancel()
        }
    }
    
    private func loadImage() async {
        if let cached = ImageMemoryCache.shared.image(for: url) {
            print("🟢 Imagen desde MEMORY cache")
            self.image = cached
            return
        }
        
        do {
            let request = URLRequest(
                url: url,
                cachePolicy: .returnCacheDataElseLoad,
                timeoutInterval: 30
            )
            
            let (data, _) = try await URLSession.shared.data(for: request)
            
            guard !Task.isCancelled else { return }
            
            let decoded = await UIImage.decode(from: data)
            
            guard let decoded, !Task.isCancelled else { return }
            
            ImageMemoryCache.shared.insert(decoded, for: url)
            
            await MainActor.run {
                withAnimation(.easeIn(duration: 0.2)) {
                    self.image = decoded
                }
            }
        } catch {
            // Cancelado o error de red, no hacemos nada
        }
    }
}
